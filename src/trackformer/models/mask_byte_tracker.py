...
class BYTETracker(object):
    ...
    def __init__(self, obj_detector, obj_detector_post, tracker_cfg,
        ...
        # 阈值初始化
        self.detection_obj_score_thresh = tracker_cfg['detection_obj_score_thresh']             # 高阈值
        self.detection_obj_score_low_thresh = tracker_cfg['detection_obj_score_low_thresh']     # 低阈值
    ...
    def step(self, blob):
        ...
        if "segm" in self.obj_detector_post:
            det_results = self.obj_detector_post['segm'](
                det_results,
                outputs,
                orig_size,
        ...
        if 'masks' in det_result:
            det_result['masks'] = det_result['masks'].squeeze(dim=1)
        ...
        if num_prev_track:
            ...
            if 'masks' in det_result:
                track_masks = det_result['masks'][:-self.num_object_queries]
        ...
        if 'masks' in det_result:
            new_det_masks_high = det_result['masks'][-self.num_object_queries:].cpu()
            new_det_masks_low = det_result['masks'][-self.num_object_queries:].cpu()
        ...
        if 'masks' in det_result:
            new_det_masks_high = new_det_masks_high[det_keep2_high].cpu()
            
        #public_detections_mask = self.public_detections_mask(
        #    det_boxes2_high, blob['dets'][0])[det_keep2_high].cpu()
        
        #det_scores2_high = det_scores2[public_detections_mask]
        #det_boxes2_high = det_boxes2[public_detections_mask]
        #det_hs_embeds_high = hs_embeds2[public_detections_mask]
        ...
        reid_mask2_high = self.reid(
            ...
            det_scores2_high,
            det_hs_embeds_high,
            new_det_masks_high if 'masks' in det_result else None, None).cpu()
        ...
        if 'masks' in det_result:
            new_det_masks_high = new_det_masks_high[reid_mask2_high]
        ...
        if 'masks' in det_result:
            new_det_masks_low = new_det_masks_low[det_keep2_low].cpu()
        ...
        reid_mask2_low = self.reid(
            ...
            det_boxes2_low,
            det_hs_embeds_low,
            new_det_masks_low if 'masks' in det_result else None, None
        ).cpu()
        ...
        new_det_masks = torch.cat((new_det_masks_high, new_det_masks_low[reid_mask2_low]), dim=0)
        ...
        new_track_ids = self.add_tracks(
            ...
            det_scores_new,
            det_hs_embeds_new,
            new_det_masks if 'masks' in det_result else None, None
        )
        ...
        if 'masks' in det_result and self.tracks:
