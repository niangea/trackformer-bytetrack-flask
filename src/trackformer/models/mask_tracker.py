...
class Tracker:
    ...
    def step(self, blob):
        ...
        if 'masks' in result:
            new_det_masks = result['masks'][-self.num_object_queries:].cpu()
        if self.generate_attention_maps:
        ...
        new_det_keep = torch.logical_and(
            ...
            result['labels'][-self.num_object_queries:] == 0).cpu()
        ...
        reid_mask = self.reid(
            ...
            new_det_attention_maps if self.generate_attention_maps else None).cpu()
